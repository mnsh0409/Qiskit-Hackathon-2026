...
| R041 | 3-arm observable reconstruction; shadow-exclusive = non-commuting observables | `evidence/observable_reconstruction.json`; `evidence/observable_reconstruction_console.txt` | `python evidence/scripts/observable_reconstruction.py` (~3 min) |
| R040 | protocol ladder; shadow inversion beats naive post-selection of same data 3.36x | `evidence/protocol_escalation.json` | `python evidence/scripts/protocol_escalation.py` (~3 min) |
| R039 | direct-Z vs shadow: direct Z is 1.56x cheaper for Q, but blind to 4/9 terms of H | `evidence/direct_z_ablation.json` | `python evidence/scripts/direct_z_ablation.py` (~1 min) |
| R038 | **PROTOTYPE, next-phase** — DOS from the same pipeline; weights become degeneracies | `evidence/dos_prototype_result.json`; `evidence/dos_console_output.txt`; `evidence/dos_signals.npz` | `python evidence/scripts/dos_prototype.py` (~7 min) |
