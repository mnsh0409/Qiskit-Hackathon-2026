...
accurate than our arm, confirmed on an independent run over a 2×-extended time window (R059
measured a similar-magnitude gap on that separate job — not a precisely comparable ratio,
since the two runs are different device-calibration snapshots, but the same conclusion).
The *Hilbert–Schmidt test* answers a state-independent
...
| 27× | how much the 2-gate echo beats our arm on hardware, confirmed over an extended window | R044, R059 |
