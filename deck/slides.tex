...
\begin{frame}{Backup --- where SKQD helps, and why not here}
  \vspace{-2mm}
  \begin{center}
    \includegraphics[width=0.82\linewidth]{fig_skqd_boundary.png}
  \end{center}
  \vspace{-3mm}
  {\scriptsize
  ...
  \par\smallskip
  \textbf{Real physics, or our toy model?} Repeated on the \hl{2D Fermi--Hubbard model}
  (2$\times$3, 12 qubits, half filling), where the knob is $U/t$: same boundary ---
  \good{12.5\%} of the sector at $U/t{=}16$ (Mott), \bad{never} for $U/t\leq4$ (metallic).
  That is why SQD succeeds in chemistry and not here. \src{R050}}
\end{frame}
