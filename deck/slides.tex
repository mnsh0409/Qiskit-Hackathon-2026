...
\author{\textbf{Team 8}\\ {\footnotesize Chao Hsien \and Jiwan Kang \and Ng Siu Hin \and Su Wei-Chang \and Tina Tien}}
...
\begin{frame}{Team 8 --- Garbage Collectors}
  \begin{itemize}
    \item \textbf{Chao Hsien} --- opens: headline scoreboard, five tracks completed (M1)
    \item \textbf{Jiwan Kang} --- the question, anti-control circuit, Track B validation (M2)
    \item \textbf{Ng Siu Hin} --- classical shadows, baselines vs.\ the echo and HST (M3)
    \item \textbf{Su Wei-Chang} --- AQC compilation win, the phase trap and its fix, hardware falsification (M4)
    \item \textbf{Tina Tien} --- Track B on hardware, claims \& scope, reproducibility, summary (M5)
  \end{itemize}
  \vfill
  \centering
...
\begin{frame}{Backup --- future work, already prototyped: density of states}
  ...
  \]
  \small
  Our $\chi(t)=\mathrm{Tr}[U(t)\rho]$. Set $\rho = \mathds{1}/2^n$ and \hl{the same
