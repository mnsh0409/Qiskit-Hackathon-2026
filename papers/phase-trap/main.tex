...
% R060
We encountered the same magnitude-hides-phase failure mode twice more in independent
settings, including a hardware run whose magnitude-based figure of merit scored a
completely decohered circuit as the best performer. Finally, we report a pre-registered
test of the compilation advantage on two superconducting processors: it does not survive
present-day error rates at $n=6$, falling short of its own prediction by a factor of
twelve, and we quantify the effective error rate at which it becomes measurable.
% R053, R054
\end{abstract}
...
(non-interferometric) uses of AQC are unaffected (Sec.~\ref{sec:scope}). Third, we
practise what the finding preaches: we pre-registered survival predictions for the
compiled Hadamard test on two superconducting processors and report their falsification,
including a second, independent appearance of the magnitude-hides-phase failure in the
hardware data itself (Sec.~\ref{sec:hardware}).
...
\section{Pre-registered hardware test, and its honest failure}
\label{sec:hardware}
...
% TODO(team): verify all arXiv ids / DOIs before submission.
...
ACM Trans. Quantum Comput. \textbf{3}, 7 (2022); arXiv:2106.05649.
