...
% R060
We demonstrate the trap directly on two superconducting processors: shipped with and
...
$\approx$186 two-qubit gates of device noise coherently, to within $0.0035$--$0.14$
rad of its predicted value --- and the devices are found to add coherent phase errors
of their own, equally invisible to magnitude-based certification: the two processors
are statistically identical by survival yet differ tenfold in phase error.
% R061, R063
...
% R061
The mechanism behind the tightness of P1 and P2 is worth stating: the correction
transpiles to an $R_z$ on the ancilla, which this hardware implements as a
\emph{virtual}-$Z$ --- a software frame update, not a physical pulse. The two arms are
therefore the same pulse sequence up to an exactly-applied frame rotation, so identical
survivals (P1) and a precisely surviving inter-arm phase (P2) are what the platform's
gate model predicts; the residual $0.06$--$0.13$\,rad is statistics and state
preparation and measurement error. That is not a weakness of the demonstration but its
point: the fix is free \emph{and} exact at the pulse level, while remaining invisible
to every magnitude-based certificate --- and the same reasoning says P1/P2 hold on any
virtual-$Z$ platform, which is why we do not present them as device-specific findings.
% R061
...
Repeating the identical job on a second processor (\texttt{ibm\_kingston})
reproduced P1 and P2 --- the latter to $0.0035$\,rad at $t=0.3$ --- and resolved part
of the P3 picture: the \emph{shallow} arm's device phase is device-dependent
(kingston's exact arm is nearly phase-clean, $+0.02$ to $+0.07$\,rad, an order of
magnitude below marrakesh's) \emph{while the two devices' survivals are statistically
identical} ($0.78$--$0.89$ on both). Judged by every magnitude metric the two devices
are the same machine; judged by phase, one is ten times cleaner --- a fourth,
unplanned instance of the thesis, now at the level of comparing devices. The deep
arms' phase error remains large on both processors ($2.1$--$2.9$\,rad in magnitude)
and is strongly time-dependent on kingston, ruling out a static ancilla detuning as
the sole mechanism; its cause remains unattributed (one run per device and time, no
repeats). The actionable conclusion is unchanged and strengthened: coherent phase
errors --- from the compiler \emph{and} from the device, on every device tested ---
demand phase-sensitive certification.
% R061, R063
